-- Añade a T. Shadow y su Vacío devorador al modo versus online.
...
alter table public.versus_match_players
  ...
  check (active_effect is null or active_effect in ('roots', 'roar', 'shuffle', 'invert', 'black_hole'));
...
begin
  ...
  if p_character_key not in ('explorador', 'mago', 'guardiana', 'dragon', 'hombre_lobo', 't_shadow') then
...
begin
  ...
  where room_id = p_room_id and user_id = v_user_id;
  if v_character not in ('explorador', 'guardiana', 'dragon', 'mago', 'hombre_lobo', 't_shadow') then
  ...
  if v_character = 'explorador' then
    ...
    v_expires_at := now() + case v_character
      when 'mago' then interval '5 seconds'
