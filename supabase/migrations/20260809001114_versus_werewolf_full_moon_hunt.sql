-- Añade al Hombre Lobo y su Inversión Lunar al modo versus online.
...
alter table public.versus_match_players
  ...
  check (active_effect is null or active_effect in ('roots', 'roar', 'shuffle', 'invert'));
...
begin
  ...
  if p_character_key not in ('explorador', 'mago', 'guardiana', 'dragon', 'hombre_lobo') then
...
begin
  ...
  if v_character not in ('explorador', 'guardiana', 'dragon', 'mago', 'hombre_lobo') then
  ...
  if v_character = 'explorador' then
    ...
    v_expires_at := now() + case v_character
      ...
      when 'hombre_lobo' then interval '4 seconds'
...
revoke execute on function public.activate_versus_ability(uuid)
  from public, anon;
grant execute on function public.activate_versus_ability(uuid)
  to authenticated;
