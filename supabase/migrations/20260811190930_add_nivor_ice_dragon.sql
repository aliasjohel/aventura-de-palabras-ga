-- Incorpora a Nivor y su Invierno absoluto al modo versus online.
...
alter table public.versus_match_players
  ...
  check (
    ...
    or active_effect in ('roots', 'roar', 'shuffle', 'invert', 'black_hole', 'key_bounce', 'ice_screen')
  );
...
begin
  ...
  if p_character_key not in (
    'explorador', 'mago', 'guardiana', 'dragon', 'hombre_lobo', 't_shadow', 'guardian_alba', 'dragon_hielo'
  ) then
...
begin
  ...
  if v_character not in (
    'explorador', 'guardiana', 'dragon', 'mago', 'hombre_lobo', 't_shadow', 'guardian_alba', 'dragon_hielo'
  ) then
  ...
  if v_character = 'explorador' then
    ...
    v_effect := case v_character
      ...
      when 'dragon_hielo' then 'ice_screen'
      else 'shuffle'
    end;
