...
assert.match(app, /const cuadrosVisibles = nubelunEnMovil \? \[faunaMision\.src\] : faunaMision\.cuadros/);
...
assert.match(css, /@media \(max-width: 640px\)[\s\S]*?\.nubelun-cumbres\s*\{\s*left:\s*80%;\s*bottom:\s*49%;\s*width:\s*7%/);
