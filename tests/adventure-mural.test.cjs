...
assert.match(app, /musicaMuralDragon = new Audio\("assets\/sounds\/mural-dragon1\.mp3"\)/);
assert.match(app, /completarRompecabezasMuralSantuario\(\)[\s\S]+reproducirMusicaCinematica\([\s\S]+musicaMuralDragon/);
assert.match(app, /sonidos\.piedra\.volume = 0\.22;[\s\S]+reproducirSonido\("piedra"\);[\s\S]+detenerSonido\("piedra"\)/);
assert.doesNotMatch(app, /reproducirSecuenciaSonidos\(\["piedra", "victoria"\]\)/);
...
assert.match(serviceWorker, /CACHE_NAME = `\$\{CACHE_PREFIX\}v100`/);
