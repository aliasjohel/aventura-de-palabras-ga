...
assert.match(estilos, /max-height: 500px[\s\S]+data-personaje="mago"[^}]+translateY\(-18%\) scale\(-2, 2\)/);
...
assert.match(app, /function vibrarAtaqueVersus\(/);
assert.match(app, /evento\.type === "word_complete"[\s\S]+vibrarAtaqueVersus\(\)/);
assert.match(estilos, /\.modal-configuracion/);
assert.match(estilos, /translateY\(-18%\) scale\(-2, 2\)/);
