...
test('rechaza insultos, tildes, separadores, repeticiones y sustituciones frecuentes',()=>{
 for(const alias of ['Pingudo 23 cm','pingudo','PÍNGUDO','p1ngud0','p.i.n.g.u.d.o','pinguda','verga','puto','PÚTÁ','p.u.t.o','p u t a','p\u200bu\u200bt\u200bo','p3lotudo','b0lud0','puto123','elputo','soy puto','puuutooo','mierda','hijo de puta','HDP','forro','coño','f.u.c.k','sh1t','asshole']){
...
test('permite nombres legítimos y palabras con coincidencias parciales',()=>{
 for(const alias of ['Pingüino','Vergara','Maricarmen','Semaforo','Ana','José','María','Ñandú','Aren','Kairos','Concha','Armando','Scunthorpe','Computadora','Diputado','Disputo','Gabi123','T-Shadow','Dragón_77']){assert.equal(aliasInapropiado(alias),false,alias);assert.equal(validarAlias(alias),alias);}
});
